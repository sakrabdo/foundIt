...
foreach($fetchAllData as $row)
{
	...
	$createTable .= '<td><p style="color:#FACF39; margin-bottom:5px"> isPublished:</p>'.$row['isPublished'].'</td>';
     ...
     $createTable .= '<tr>';
	$createTable .= '<td>  <a href="adminDeleteUser.php?Duserid='.$userid.'" class="item_contact transition">Delete</a> </td>';
     $createTable .= '</tr>';	

     $createTable .= '<tr>';
	$createTable .= '<td>  <a href="updatePass.php?userpass='.$userid.'" class="item_contact transition">Update Password</a> </td>';
     $createTable .= '</tr>';	
